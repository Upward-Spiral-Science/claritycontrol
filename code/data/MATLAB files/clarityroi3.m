...
%loop through all ROIs
    for i = 1:13 %loop through all features
        figure(i);
        for j=1:12
        X = [brain_173(roi1,i+6), brain_174(roi1,i+6), brain_175(roi1,i+6), brain_177(roi1,i+6) ,brain_178(roi1,i+6), brain_181(roi1,i+6),brain_182(roi1,i+6),brain_189(roi1,i+6),brain_239(roi1,i+6),brain_258(roi1,i+6);brain_173(roi1,j+7), brain_174(roi1,j+7), brain_175(roi1,j+7), brain_177(roi1,j+7) ,brain_178(roi1,j+7), brain_181(roi1,j+7),brain_182(roi1,j+7),brain_189(roi1,j+7),brain_239(roi1,j+7),brain_258(roi1,j+7)];
        X=X.';
        idx(:,j) = kmeans(X,2);
       s(:,j) = silhouette(X,idx(:,j),'cityblock');
       subplot(3,4,j);plot(X(1:5,1),X(1:5,2),'mo','MarkerSize',8);hold on;
       plot(X(6:10,1),X(6:10,2),'co','MarkerSize',8);hold on;
       plot(X(idx(:,i)==1,1),X(idx(:,i)==1,2),'*r');hold on;
       plot(X(idx(:,i)==2,1),X(idx(:,i)==2,2),'*b');
